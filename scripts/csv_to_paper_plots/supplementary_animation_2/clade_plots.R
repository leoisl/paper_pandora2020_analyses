...
nb_of_samples <- args[3]
plot_title <- paste("Number of samples = ", nb_of_samples, sep="")
output_file <- args[4]
###############################################################################################
...
for (current_ref in ref_ordering) {
  if (!identical(current_ref, "PRG")) {
    ...
    plot <- ggplot(data = recall_table_for_ref, aes(x=sample, y=recalls_wrt_truth_probes, fill=sample)) +
        ...
        geom_line(data=recall_table_for_pandora_with_denovo, aes(x=sample, y=recalls_wrt_truth_probes, group=1), colour = "black", size = 2) +
        ggtitle(current_ref) +
        theme(
          ...
          axis.text.y = if(index%%5==1) element_text(size=30) else element_blank(),
